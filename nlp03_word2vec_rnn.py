...
class MyRNN(nn.Module):
    def __init__(self, vocab_size, embed_size=100, rnn_hidden_size=80, rnn_num_layer=1, output_size=20):
        super(MyRNN, self).__init__()
        self.embed = nn.Embedding(vocab_size, embed_size)
        self.drop  = nn.Dropout(0.2)
        self.rnn   = nn.RNN(embed_size, rnn_hidden_size, rnn_num_layer, batch_first=True)
...
if __name__ == '__main__':
    ...
    train_indices = [torch.LongTensor(doc2index(doc)[:DATA_MAX_LEN]) for doc in train_raw.data]
    tests_indices = [torch.LongTensor(doc2index(doc)[:DATA_MAX_LEN]) for doc in tests_raw.data]
    train_tensors = nn.utils.rnn.pad_sequence(train_indices, batch_first=True)
