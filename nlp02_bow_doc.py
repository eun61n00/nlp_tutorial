...
def build_docfreq(docs, vocab):
    docfreq = np.zeros(len(vocab), dtype=np.int64)
    for idx, doc in enumerate(docs):
        hit_vector = doc2bow_hit(doc, vocab)
        docfreq += hit_vector
    return np.array(docfreq)[0] # Make 'np.matrix' to 'np.array'

def build_idf(docs, vocab):
    df = build_docfreq(docs, vocab)
    n = len(docs)
...
def doc2bow_tfidf(doc, vocab, idf):
    tf = doc2bow_count(doc, vocab)
    return tf.multiply(idf).tocsr() # TF * IDF
...
if __name__ == '__main__':
    ...
    print(head)
    print(sept)
    _, nonzero_cols = doc_vectors[-1]['vector'].nonzero() # Ignore the row indices of non-zero elements (all 0)
    for idx in nonzero_cols:
        text = f'| {vocab_inv[idx]:12} | '
        for dv in doc_vectors:
            text += f"{dv['vector'][0,idx]:>6.3f} | "
        print(text)
